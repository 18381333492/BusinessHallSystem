...
namespace DapperExtensions
{
    public interface IDapperImplementor
    {
        ...
        int Update<T>(IDbConnection connection, T entity, IDbTransaction transaction, int? commandTimeout) where T : class;
    ...
    internal class DapperImplementor : IDapperImplementor
    {
        ...
        public void Insert<T>(IDbConnection connection, IEnumerable<T> entities, IDbTransaction transaction, int? commandTimeout) where T : class
        {
            ...
            string sql = SqlGenerator.Insert(classMap,string.Empty,string.Empty);
        ...
        public dynamic Insert<T>(IDbConnection connection, T entity, IDbTransaction transaction, int? commandTimeout) where T : class
        {
            ...
            //自定义特性的映射
            var SeqArray = identityColumn.PropertyInfo.GetCustomAttributes(true);
            string SeqName=string.Empty;
            if (SeqArray.Count() == 1)
            {
                var attr = SeqArray[0] as SEQAttribute;
                SeqName=attr.Name;
            }
           ...
           // IDictionary<string, object> keyValues = new ExpandoObject();
            string sql = SqlGenerator.Insert(classMap, SeqName, identityColumn.Name);
            if (identityColumn != null)
            {
                ...
                {
                    code=connection.Execute(sql, entity, transaction, commandTimeout, CommandType.Text);
                    sql = SqlGenerator.IdentitySql(SeqName);
                    result = connection.Query<long>(sql, entity, transaction, false, commandTimeout, CommandType.Text);
                }
        ...
        public int Update<T>(IDbConnection connection, T entity, IDbTransaction transaction, int? commandTimeout) where T : class
        {
            IClassMapper classMap = SqlGenerator.Configuration.GetMap<T>();
